...
**Scanner v3 (LiDAR paperless) — groundwork in progress since 2026-06-12** (v2's full validation was the gate; see [.claude/decisions/scanner-v3-depth-2026-06.md](.claude/decisions/scanner-v3-depth-2026-06.md)). Pure depth math lives in `lib/scanner/depth/` (point cloud → RANSAC floor plane → height-band foot segmentation → the same validated `widthAcrossFootBand`), behind a `DepthAdapter` boundary mirroring `VisionAdapter` — only a future `arkitDepthAdapter.ts` may import the native ARKit bridge. The native ARKit bridge (`modules/footfit-vision/ios/FootfitDepthModule.swift` — sceneDepth + confidence map + live `FootfitDepthARView` height-coach) and a dev-only `DepthDebugScreen` (Home → dashed blue entry) exist. Width is solved (calibrated 108–113 across stance/floor/light). Length is stance-dependent: ruler-grade (261 × 109 vs 263) only with a vertical shin. Device-found failure modes (other-foot/shin clustering, black-fabric invented depth, sub-10 mm toes, leg occlusion shadow) each have unit-tested defences. **Trust gate shipped 2026-06-16 (reject, don't repair):** confidence = `floorScore × footScore × heelScore`, where `heelScore` rejects a leaning leg's thin-tail rear (`rearBandWidth` < 20 mm) vs a real heel (≥45 mm); frames below `TRUST_MIN_CONFIDENCE_V3 = 0.8` are dropped from the burst median. Validated against five real *negative* fixtures (`lib/__tests__/scanner/depth/fixtures/`, `depthTrustGate.test.ts`); a second failure mode surfaced — too much leg in frame + forefoot confidence-dropout → stubby under-reads, fixable only capture-side (aim coaching), not by threshold tuning. **Aim coaching shipped 2026-06-17 (`787e389`):** `DepthDebugScreen` now leads with an `AIM_CUE` ("aim straight down at your foot, not along your leg — keep your shin out of frame") targeting that failure mode directly; stance cue demoted to a secondary line. **Confidence-aware extremity recovery shipped 2026-06-17:** the dominant length under-read came from the thin near-floor extremities (toe tips, heel pad) returning LOW confidence and being dropped, truncating length tip-to-tip. `recoverExtremities` (`footFromDepth.ts`) + `depthFrameLowConfPoints` (`pointCloud.ts`) region-grow the foot out from its high-conf core, admitting only contiguous, near-floor (≤25 mm), in-width-envelope low-conf points (two passes: ball-centred reach sets length so the wide floor-fan beyond the toe can't extend it; a locally-recentred fill restores the eroded heel's width). Replay-validated on all 6 good-pose + 5 negative fixtures: **heel-erosion length is fixed (frame6 248→263; goodpose-12 190→224, now trusted), width untouched, and the 5 negatives stay byte-identically rejected (`depthTrustGate.test.ts` unchanged).** Honest residual: **toe-dropout frames (goodpose-28/19) have no toe in the depth data beyond the core (only a wide floor-fan, correctly rejected) so they still read ~221/241 — a CAPTURE limit (the aim-down cue targets it), not math; and frame6/goodpose-53 stay conservatively rejected because their reconstructed heel is too close to a leaning frame's to separate (recovery deliberately does not loosen the trust gate).** **2026-07-23: shin-brace capture protocol validated** (phone's top edge braced against the shin — leg physically behind the lens): first TRUSTED frames ever (`shinbrace-16-48-*` fixtures). Toe tips proved present in the depth bytes at 3–5 mm HIGH confidence — below the 6 mm segmentation floor — so recovery gained a sub-band tip input; `trimLegShadow` keeps an occluded heel's wide+low rear slices; a toe-presence signal (`toeScore`) joined the confidence product. Trusted frames read 249/254 vs truth 263 — the residual is probed occlusion (ankle hides ~10 mm of rear heel) + sub-2 mm tips, NOT math. Next: brace the camera a few cm forward of the ankle; do not tune thresholds against these fixtures. See the 2026-07-23 section of the ADR. Read [.claude/decisions/scanner-v3-depth-2026-06.md](.claude/decisions/scanner-v3-depth-2026-06.md) before touching `lib/scanner/depth/*`.

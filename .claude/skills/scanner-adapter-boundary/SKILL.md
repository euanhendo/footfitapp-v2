...
description: Enforces the VisionAdapter boundary when editing lib/scanner/* or the scanner screens (app/screens/ScannerScreen.tsx, app/screens/ScanReviewScreen.tsx). Bans direct `react-native-fast-tflite`, `expo-camera`, or ML-runtime imports from inside lib/scanner/* and mandates dependency injection for testability. Use when touching scanner math, camera capture, or CV model integration.
---
...
Only **concrete adapter implementations** (e.g. `lib/scanner/tfliteVisionAdapter.ts`, when it lands) know that a native ML runtime exists. `lib/scanner/*` math modules and `app/screens/Scanner*.tsx` screens consume a `VisionAdapter` via dependency injection. No one else touches TFLite, MediaPipe, or native camera APIs.
...
```ts
// lib/scanner/tfliteVisionAdapter.ts (concrete, added in phase 3)
export const tfliteVisionAdapter: VisionAdapter = { /* uses react-native-fast-tflite */ };

// app/screens/ScannerScreen.tsx
import { tfliteVisionAdapter } from '../../lib/scanner/tfliteVisionAdapter';
const metrics = computeFootMetrics(await tfliteVisionAdapter.segmentFoot(uri), pxPerMm);
```

Screens **never** `import * as tflite from 'react-native-fast-tflite'` directly. If you see that import outside a concrete `*VisionAdapter.ts` file, it's a bug.
...
Never `jest.mock('react-native-fast-tflite')` — same reasoning as for SecureStore: couples tests to native-module internals and breaks CI.
...
- `import * as tflite` / `from 'react-native-fast-tflite'` in any `app/screens/*.tsx` file or in `lib/scanner/{calibration,footMetrics,referenceObjects,types,visionAdapter}.ts`.
...
- `jest.mock('react-native-fast-tflite')` in a test.
